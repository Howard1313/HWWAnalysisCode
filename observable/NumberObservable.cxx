#include "HWWAnalysisCode/NumberObservable.h"
#include <limits>
...
//#include "xAODParticleEvent/CompositeParticleContainer.h"
//______________________________________________________________________________________________
double NumberObservable::getValue() const {
  ...
  */
  ...
  if (!this->fEvent->retrieve(evtInfo, "EventInfo").isSuccess()) {
    DEBUGclass("Failed to retrieve event info!");
    return false;
  }
  DEBUGclass("runNumber = %i, eventNumber = %i ", evtInfo->runNumber(),evtInfo->eventNumber());
  const bool isSim = evtInfo->eventType(xAOD::EventInfo::EventType::IS_SIMULATION);
  DEBUGclass("isSim:%s", isSim ? "true" : "false");
  DEBUGclass("this->mCandName:%s",this->mCandName.Data());
  DEBUGclass("evtInfo->isAvailable(RandomRunNumber):%i",evtInfo->isAvailable<unsigned int>("RandomRunNumber"));
//evtInfo->auxdata<char>(std::string(m_trigpass_prefix+trigger.Data()))
  TString m_trigpass_prefix= "pass_";
  TString trigger="HLT_mu50";
  DEBUGclass("trig string:%s",trigger.Data());
  DEBUGclass("trig string:%s",std::string(trigger.Data()));
  // DEBUGclass("trig string:%s",std::string(m_trigpass_prefix+trigger.Data()));
  // DEBUGclass("auxdata<char>:%s",evtInfo->auxdata<char>(std::string(m_trigpass_prefix+trigger.Data())));
  
    
//1st loop
  DEBUGclass("============================ NumberObservable info start ===============================");
  float sumPt_SRjet=0;
  ...
  switch (fnumber_variable) {
    ...
    case Number_Variable::sumPt_SRjet: 
        ...
        break;
    case Number_Variable::Wjj_count: 
        rval = Wjj_count;
        break;
    default:
...
// TQObservable* NumberObservable::getClone() const  {
...
//______________________________________________________________________________________________
bool NumberObservable::initializeSelf(){
  TString CandName = "";
...
//______________________________________________________________________________________________
NumberObservable::NumberObservable(const TString& name,const int number_variable):TQEventObservable(name)
{
  fnumber_variable=static_cast<Number_Variable>(number_variable);
}

bool NumberObservable :: cmp_pt_IParticle (std::tuple<const xAOD::IParticle*, int> a,std::tuple<const xAOD::IParticle*, int> b)
{
    return std::get<0>(a)->pt() > std::get<0>(b)->pt();
}
bool NumberObservable :: cmp_pt_Muon (const xAOD::Muon* a,const xAOD::Muon* b)
{
    return a->pt() > b->pt();
}
bool NumberObservable :: cmp_pt_Electron (const xAOD::Electron* a,const xAOD::Electron* b)
{
    return a->pt() > b->pt();
}
bool NumberObservable :: cmp_pt_TLorentzVector (TLorentzVector a,TLorentzVector b)
{
    return a.Pt() > b.Pt();
}
bool NumberObservable :: cmp_pt_Jet (const xAOD::Jet* a,const xAOD::Jet* b)
{
    return a->pt() > b->pt();
}
