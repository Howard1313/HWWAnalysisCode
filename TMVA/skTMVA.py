...
def convert_bdt__AdaBoost(sklearn_bdt_clf, input_var_list, tmva_outfile_xml):
    ...
    for ind, val in enumerate(var_list):
        ...
        print(var_min)
        print(str(var_min))
        Variable = ET.SubElement(Variables, "Variable", VarIndex=str(ind), Type=val[1],Expression=name, Label=name, Title=name, Unit="", Internal=name, Min=str(var_min), Max=str(var_max))
